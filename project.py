...
def deleteItem(item_id): #REQUIRES TESTING
	...
	if request.method == 'POST':
		...
		return render_template('deleteitem.html', item = itemToDelete, authorized = authorized)
...
def gconnect():
	...
	try:
		...
		response = make_response(json.dumps('Failed to upgrade to authorization code'), 401)
	...
	output += '<img src="%s" style="width:300px; height: 300px; border-radius: 150px; -webkit-border-radius: 150px; -moz-border-radius: 150px;">' % login_session['image']
...
def gdisconnect():
	if 'access_token' not in login_session:
		response = make_response(json.dumps('Access token is not in session'), 401)
		response.headers['Content-Type'] = 'application/json'
		return response
...
def fbdisconnect():
	if 'access_token' not in login_session:
		response = make_response(json.dumps('Access token not in session'), 401)
		response.headers['Content-Type'] = 'application/json'
		return response
